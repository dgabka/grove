...
- `grove switch` — select any other running tmux session, most recently used first (least recently used at the bottom). Use `grove switch --repo` inside a Git repository to show only Grove sessions with the same repository identity; outside a repository it shows all sessions.
